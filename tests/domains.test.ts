...
import { lex } from '../src/lexer';  // Fix: Added for end-to-end test
import { parse } from '../src/parser';  // Fix: Added
import { resolve } from '../src/resolver';  // Fix: Added
import { compileRules } from '../src/compiler';  // Fix: Added
import { evaluate } from '../src/evaluator';  // Fix: Added
...
describe('NSML Domain Hooks', () => {
  ...
  it('should allow custom registration', () => {
    ...
    const handler = (node: AstNode, context: SymbolTable) => ({ result: 'custom' });
    registerDomain(customType, handler);
    expect(domainRegistry.has('custom')).toBe(true);
    const result = domainRegistry.get('custom')!({ type: 'custom', attributes: {}, children: [], line: 1 }, new Map());
  ...
  it('should integrate chess hook in full evaluation', () => {
    ...
    expect(result.results.chessTest).toEqual({ board: ['e4', 'e5'], moves: ['e2-e4'] });
  });
});