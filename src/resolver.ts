...
export function resolve(ast: AstNode | null): ResolveResult {
  ...
  function resolveSymbol(
    ...
    } else if (entry.type === 'number' && isNaN(entry.value)) {
      errors.push({
        type: 'semantic',
        message: `Type mismatch for '${name}' (invalid number)`,
        line: node.line,
        suggestedFix: `Provide a valid number value`,
      });
    }
...
export function parseValue(val: string, type: SymbolType): any {
