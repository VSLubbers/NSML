...
export function resolve(ast: AstNode | null): ResolveResult {
  ...
  if (!ast) {
    errors.push({ type: 'semantic', message: 'Invalid or null AST from parser', suggestedFix: 'Check parsing stage for errors' });
  ...
  function resolveSymbol(node: AstNode, symbols: SymbolTable, errors: EvalError[]) {
    ...
    if (!name) {
      errors.push({ type: 'semantic', message: 'Missing name attribute', line: node.line, suggestedFix: 'Add name="..." to the element' });
      return;
    }
    if (symbols.has(name)) {
      errors.push({ type: 'semantic', message: `Duplicate symbol '${name}'`, line: node.line, suggestedFix: 'Rename or remove duplicate' });
      return;
    }
    ...
    if (attrType) {
      if (allowedSymbolTypes.includes(attrType as SymbolType)) {
        ...
        errors.push({ type: 'semantic', message: `Invalid type '${attrType}' for '${name}'`, line: node.line, suggestedFix: `Use one of: ${allowedSymbolTypes.join(', ')}` });
        return;
      }
    }
    ...
    switch (node.type) {
      case 'var':
        entry = {
          kind: 'var',
          type: resolvedType,
          value: parseValue(node.attributes.init || node.text || 'undefined', resolvedType, symbols, errors, node.line),
          mutable: true,
        };
        break;
      case 'const':
        entry = {
          kind: 'const',
          type: resolvedType,
          value: parseValue(node.attributes.value || node.text || 'undefined', resolvedType, symbols, errors, node.line),
          mutable: false,
        };
        break;
      case 'set':
        entry = {
          kind: 'set',
          type: 'set',
          value: new Set(node.attributes.elements?.split(',').map(v => parseValue(v.trim(), 'number', symbols, errors, node.line)) || []),
          mutable: true,
        };
        break;
      case 'graph':
      ...
      case 'entity':
        ...
        node.attributes.props?.split(',').forEach(p => {
          ...
          if (!key || val === undefined) {
            errors.push({ type: 'semantic', message: `Invalid prop '${p}' for '${name}'`, line: node.line, suggestedFix: 'Use format key=value' });
            return;
          }
        ...
        break;
      default:
        errors.push({ type: 'semantic', message: `Unknown symbol type '${node.type}'`, line: node.line, suggestedFix: 'Use valid symbol tag like <var> or <const>' });
        return;
    }
    ...
    if (entry.type !== 'any' && ['number', 'string', 'boolean'].includes(entry.type) && typeof entry.value !== entry.type) {
      ...
      if (!(entry.type === 'number' && isNaN(entry.value))) {
        errors.push({ type: 'semantic', message: `Type mismatch for '${name}'`, line: node.line, suggestedFix: `Change value to match type '${entry.type}'` });
      }
    }
...
export function parseValue(val: string, type: SymbolType, symbols: SymbolTable, errors: EvalError[], line: number): any {
  ...
  if (isIdentifier(val)) {
    if (symbols.has(val)) {
      ...
      if (type !== 'any' && refEntry.type !== type) {
        errors.push({ type: 'semantic', message: `Reference type mismatch for '${val}' (expected ${type}, got ${refEntry.type})`, line, suggestedFix: 'Ensure referenced symbol matches type' });
        return undefined;
      }
      return refEntry.value;
    } else {
      errors.push({ type: 'semantic', message: `Unresolved reference '${val}'`, line, suggestedFix: 'Define the symbol before referencing it' });
      return undefined;
    }
  }
  ...
  switch (type) {
    case 'number':
      ...
      if (isNaN(num)) {
        errors.push({ type: 'semantic', message: `Invalid number value '${val}'`, line, suggestedFix: 'Provide a valid number' });
        return undefined;
      }
      return num;
    case 'boolean':
      ...
      errors.push({ type: 'semantic', message: `Invalid boolean value '${val}'`, line, suggestedFix: 'Use true or false' });
      return undefined;
    default:
      return val;
  }
